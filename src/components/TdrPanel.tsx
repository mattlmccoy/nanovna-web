...
import { computeBandpassTdr, type TdrWindow } from '../lib/tdr';
...
export function TdrPanel({ points, sourceName, onClose }: { points: SweepPoint[]; sourceName: string; onClose: () => void }) {
  ...
  const calculation = useMemo(() => {
    try { return { result: computeBandpassTdr(points, velocity, tdrWindow), error: '' }; }
    catch (error) { return { result: null, error: (error as Error).message }; }
  }, [points, velocity, tdrWindow]);
  ...
  useEffect(() => {
    ...
    const max = Math.max(...result.bins.map((bin) => bin.magnitude), 1e-12);
    for (let tick = 0; tick <= 5; tick += 1) {
      ...
      ctx.fillText(number(max * (5 - tick) / 5, 2), 3, y + 3);
    }
    ...
    result.bins.forEach((bin, index) => {
      ...
      const y = pad.top + plotHeight * (1 - bin.magnitude / max);
    ...
    const peakY = pad.top + plotHeight * (1 - peak.magnitude / max);
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}>
    <section className="about-dialog tdr-dialog" role="dialog" aria-modal="true" aria-labelledby="tdr-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="tdr-controls">
        ...
        <span>Mode <b>Bandpass reflection magnitude</b></span>
      </div>
      {calculation.error ? <div className="tdr-error">{calculation.error}</div> : calculation.result && <>
        ...
        <div className="tdr-summary"><span>Estimated strongest discontinuity <b>{number(calculation.result.estimatedLengthMeters)} m</b></span><span>Range resolution <b>{number(calculation.result.rangeResolutionMeters)} m</b></span><span>Distance bin <b>{number(calculation.result.distanceBinMeters)} m</b></span><span>Unambiguous range <b>{number(calculation.result.unambiguousRangeMeters)} m</b></span></div>
      </>}
      <p className="tdr-note">Source: {sourceName}. This is a bandpass transform of the acquired S11 samples. It does not invent DC data. Distance assumes a uniform frequency grid and the selected propagation velocity. Windowing intentionally changes sidelobes and peak width.</p>
    </section>
  </div>;
}
