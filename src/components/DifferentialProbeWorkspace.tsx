...
export function DifferentialProbeWorkspace({ points, markerIndex, context, dataFresh, sourceName, connected, busy, onToggleConnection, onStartSweeping, onStopSweeping }: { points: SweepPoint[]; markerIndex: number; context: DifferentialProbeContext; dataFresh: boolean; sourceName: string; connected: boolean; busy: boolean; onToggleConnection: () => Promise<void>; onStartSweeping: () => void; onStopSweeping: () => void; }) {
  ...
  function exportSession() {
    ...
    download(touchstone(baselinePoints, `Differential Probe baseline mean; specimen=${specimenId || 'unspecified'}; calibration=${context.calibration}`), 'text/plain', `nanovna-differential-baseline-${stamp}.s1p`);
    if (recordedFrames.length) download(touchstone(recordedFrames[recordedFrames.length - 1].trace, `Differential Probe final recorded trace; specimen=${specimenId || 'unspecified'}; calibration=${context.calibration}`), 'text/plain', `nanovna-differential-final-${stamp}.s1p`);
  }
